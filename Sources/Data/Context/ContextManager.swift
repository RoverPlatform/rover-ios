...
extension ContextManager: StaticContextProvider {
    ...
    var buildEnvironment: Context.BuildEnvironment {
        // Push registration must reflect the app's signed aps-environment rather than
        // the Xcode build configuration, so we inspect the embedded provisioning profile.
        #if targetEnvironment(simulator)
            return .development
        #else
            return loadProvisioningProfileEnvironment() ?? .production
        #endif
    }
...
extension ContextManager {
    static let provisioningProfileXMLHeader = Data("<?xml version=\"1.0\" encoding=\"UTF-8\"?>".utf8)
    static let provisioningProfilePlistClose = Data("</plist>".utf8)

    func loadProvisioningProfileEnvironment() -> Context.BuildEnvironment? {
        guard let path = Bundle.main.path(forResource: "embedded", ofType: "mobileprovision") else {
            os_log("Provisioning profile not found", log: .context, type: .info)
            return nil
        }
        ...
        return Self.provisioningProfileEnvironment(from: data)
    }

    static func provisioningProfileEnvironment(from data: Data) -> Context.BuildEnvironment? {
        guard let startRange = data.range(of: provisioningProfileXMLHeader),
            let endRange = data.range(of: provisioningProfilePlistClose, options: .backwards),
