...
public struct DataAssembler: Assembler {
    ...
    public init(
        accountToken: String, endpoint: URL = URL(string: "https://api.rover.io/graphql")!,
        engageEndpoint: URL = URL(string: "https://engage.rover.io")!, appGroup: String? = nil, flushEventsAt: Int = 20,
        flushEventsInterval: Double = 30.0,
        maxEventBatchSize: Int = 100, maxEventQueueSize: Int = 1_000
    ) {
    ...
    public func assemble(container: Container) {
        ...
        container.register(ContextProvider.self) { resolver in
            ModularContextProvider(
                ...
                liveActivityTokensContextProvider: resolver.resolve(LiveActivityTokensContextProvider.self),
        ...
        container.register(EventQueue.self) {
            [flushEventsAt, flushEventsInterval, maxEventBatchSize, maxEventQueueSize] resolver in
            EventQueue(
        ...
        container.register(ConfigManager.self, scope: .singleton) { resolver in
            return MainActor.assumeIsolatedOrFatalError {
                ConfigManager(userDefaults: UserDefaults(suiteName: self.appGroup) ?? .standard)
            }
        }

        // MARK: HomeViewManager

        container.register(HomeViewManager.self, scope: .singleton) { resolver in
            return MainActor.assumeIsolatedOrFatalError {
                HomeViewManager(
                    httpClient: resolver.resolve(HTTPClient.self)!,
                    userDefaults: UserDefaults(suiteName: self.appGroup) ?? .standard,
        ...
        container.register(ConfigSync.self, scope: .singleton) { resolver in
            return MainActor.assumeIsolatedOrFatalError {
                ConfigSync(
                    httpClient: resolver.resolve(HTTPClient.self)!,
