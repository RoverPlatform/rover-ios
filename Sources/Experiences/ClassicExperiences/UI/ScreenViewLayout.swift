    ...
    func prepare(frame: CGRect) {
        ...
        for (i, row) in screen.rows.enumerated() {
            ...
            for (j, block) in row.blocks.enumerated() {
                ...
                switch block {
                case let block as ClassicBarcodeBlock:
                ...
                case let block as ClassicButtonBlock:
                ...
                case let block as ClassicImageBlock:
                    let aspectRatio = CGFloat(block.image.width) / CGFloat(block.image.height)
                    intrinsicHeight = blockWidth / aspectRatio
                case let block as ClassicTextBlock:
                    guard let attributedText = block.text.attributedText(forFormat: .html) else {
                        intrinsicHeight = nil
                        break
                    }
                    ...
                    let measuredHeight = attributedText.measuredHeight(with: size)
                    intrinsicHeight = measuredHeight + CGFloat(block.insets.top) + CGFloat(block.insets.bottom)
                case let block as ClassicTextPollBlock:
                    intrinsicHeight = block.intrinsicHeight(blockWidth: blockWidth)
                case let block as ClassicImagePollBlock:
                    intrinsicHeight = block.intrinisicHeight(blockWidth: blockWidth)
                default:
                    intrinsicHeight = nil
                }
                
                let heightFromType: (ClassicHeight) -> CGFloat? = { heightType in
                    switch heightType {
                    case .intrinsic:
