...
class PollCell: BlockCell {
    ...
    var questionText: ClassicText? {
        didSet {
            ...
            // Accessibility
            let numberOfOptions = (block as! ClassicPollBlock).poll.pollOptions.count
    ...
    override func configure(with block: ClassicBlock) {
        ...
        guard let pollBlock = self.block as? ClassicPollBlock else {
    ...
    private var state: PollState = .unbound {
        ...
        didSet {
            ...
            case .initialState:
                ...
                guard let pollBlock = self.block as? ClassicPollBlock & ClassicBlock else {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .milliseconds(Int(delay * 1000))) {
                        self?.urlSession.fetchPollResults(for: pollBlock.pollID(containedBy: experienceID), optionIDs: pollBlock.poll.optionIDs) { pollResults in
                            DispatchQueue.main.async {
                                ...
                                let currentlyAssignedBlock = self.block as? ClassicPollBlock
