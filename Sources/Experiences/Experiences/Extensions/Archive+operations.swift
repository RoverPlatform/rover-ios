...
extension Archive {
    ...
    func extractStringsTables() throws -> StringTable {
        ...
        return try reduce(into: [:]) { result, entry in
            ...
            guard segments.count == 2, segments.first == "localization",
                let fileNameComponents = segments.last?.split(separator: ".", maxSplits: 1),
                fileNameComponents.count == 2, fileNameComponents[1] == "json"
            else {
