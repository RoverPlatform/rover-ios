...
final class ExperienceModel: Decodable {
    ...
    /// Font download URLs, deduplicated preserving order: documents authored in
    /// the Mac app can declare the same font family more than once, and each
    /// source only needs downloading and registering once.
    var fontURLs: [URL] {
        var seen = Set<URL>()
        return fonts.flatMap { $0.sources.map { $0.assetUrl } }.filter { seen.insert($0).inserted }
    }
