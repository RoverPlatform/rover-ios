...
open class ExperienceViewController: UIViewController {
    ...
    /// Hub), so no close chrome is added. Set before `loadExperience`.
    ...
    /// Loads a V3 App Screens experience: normalizes the scheme, gates the domain
    /// (mirroring `ExperienceStoreService`; kept so an invalid domain still shows the
    /// error UI), then embeds the SwiftUI `AppScreensNavigationHostView` via a
    /// `UIHostingController`. Modal presentations (a presenter supplied
    /// `onDismissButtonPressed`) get a root close item + the owner dismiss-then-open
    /// handler; embedded experiences get a clean bar and open-in-place. Teardown is the
    /// hosted `AppScreensTokenBox.deinit → release`, so no root host is stashed here.
    private func loadAppScreensExperience(with url: URL) {
        // Deep links arrive with a custom scheme; normalize to https before the
        // domain gate (mirrors the store's normalization).
        ...
        // Modal vs embedded is exactly `onDismissButtonPressed`-nil-ness (the existing
        // presented/embedded discriminator). Modal → owner branch: close item + a
        // completion-capable dismiss-then-open. Embedded → nil handler → the flow's
        // coordinator opens in place.
        let openExternalURL = makeAppScreensOpenExternalURLHandler(
            isModal: onDismissButtonPressed != nil,
            dismiss: { [weak self] completion in
                guard let self = self else {
                    completion()
                    return
                }
                // Dismiss the whole presentation (and any stacked SwiftUI sheets) via
                // the presenter, then open — matching HubHostingController.
        ...
        let hostingController = UIHostingController(
            rootView: AppScreensNavigationHostView(
                url: normalizedURL,
                onClose: onDismissButtonPressed,
                onOpenExternalURL: openExternalURL
            )
        )

        setChildViewController(hostingController)
        // `setChildViewController` sizes the child once from `view.bounds` and adds no constraints, so
        // the child never tracks its container. That is invisible for the other renderers, whose
        // content respects the safe area, but this controller is normally presented with the default
        // modal style — a `.pageSheet` — whose container is resized AFTER the child is added. The
        // hosted App Screen `.ignoresSafeArea()` and its page uses `min-height: 100vh`, so a stale
        // pre-sheet height puts the bottom of the page past the bottom of the window: measured as a
        // 62 pt overhang, which reported content as on-screen and hittable where taps are dropped.
        //
        // Scoped here rather than in `setChildViewController` deliberately: that method is shared with
        // the classic and v2 renderers, and v2's `NavBarViewController` already derives its height from
        // `modalPresentationStyle`, so changing the container geometry beneath every child risks
        // shifting layout on paths that render correctly today.
    ...
    // Retained as a no-op so `ExperienceViewController`'s open override surface stays
    // stable for subclassers. It used to also reset the App Screens nav controller,
    // but that reset now lives in the SwiftUI host (`AppScreensNavigationHostView`).
