...
extension RCHPersistentContainer {
  ...
  func fetchPostByID(uuid: UUID) -> Post? {
    ...
    do {
      ...
      os_log(
        "Failed to fetch post by UUID: %{public}@", log: .communicationHub, type: .error,
  ...
  func fetchPostByID(uuidString: String) -> Post? {
    guard let uuid = UUID(uuidString: uuidString) else {
      os_log("Invalid UUID format: %{public}@", log: .communicationHub, type: .error, uuidString)
      return nil
    }
  ...
  func getPostsCursor() -> String? {
    ...
    do {
      ...
      os_log(
        "Failed to fetch posts cursor: %{public}@", log: .communicationHub, type: .error,
        error.localizedDescription)
      return nil
    }
  }
  ...
  func updatePostsCursor(_ cursor: String?) {
    ...
    do {
      ...
      os_log(
        "Failed to update posts cursor: %{public}@", log: .communicationHub, type: .error,
        error.localizedDescription)
    }
  }
  ...
  func markPostAsRead(_ post: Post) {
    ...
    do {
      ...
      os_log(
        "Failed to mark post as read: %{public}@", log: .communicationHub, type: .error,
        error.localizedDescription)
    }
  }
  ...
  func createOrUpdatePost(from postItem: PostItem) -> Bool {
    ...
    do {
      ...
      if let subscriptionID = postItem.subscriptionID {
        ...
        if let existingSubscription = subscriptionResults.first {
          ...
          os_log(
            ...
            log: .communicationHub, type: .debug, subscriptionID)
        }
      }
      ...
      os_log(
        "Failed to create/update post: %{public}@", log: .communicationHub, type: .error,
        error.localizedDescription)
      return false
    }
  }
  ...
  func loadSampleData() {
    ...
    do {
      ...
      os_log(
        "Failed to load sample data: %{public}@", log: .communicationHub, type: .error,
        error.localizedDescription)
    }
