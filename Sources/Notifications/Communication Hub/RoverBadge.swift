...
/// This object is responsible for updating the main app badge, and also offers API for obtaining (and observing) the badge count for a communication hub tab.
@MainActor
public class RoverBadge: ObservableObject {
    ...
    /// Whether the communication hub tab has unread items and should display a badge.
    /// 
    /// If nil, then the count is 0 and the badge is not displayed.
    @Published public private(set) var newBadge: String? = nil

    init(persistentContainer: RCHPersistentContainer, updateAppBadge: Bool) {
    ...
    private func updateBadgeCount() {
        Task { @MainActor in
            ...
            if self.updateAppBadge {
                os_log("Updating app badge number to %d", log: .communicationHub, type: .info, unreadCount)
