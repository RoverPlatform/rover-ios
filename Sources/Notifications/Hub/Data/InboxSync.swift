...
actor InboxSync: ObservableObject, SyncStandaloneParticipant {
    ...
    init(persistentContainer: InboxPersistentContainer, httpClient: HTTPClient) {
        ...
        if state != .loaded {
            ...
            waitLoop: for await state in await MainActor.run(body: { persistentContainer.$state.values }) {
                ...
                case .failed:
                    ...
                    return false
                case .loaded:
                    os_log(.debug, log: .hub, "Persistence loading completed, now can sync")
                    break waitLoop
                }
            }
        }
        ...
        case .success(let response):
            await MainActor.run {
                ...
                os_log(
                    .debug,
                    log: .hub,
                    "Synced %d subscriptions",
                    response.subscriptions.count
                )
            }
...
extension HTTPClient {
    func getSubscriptions() async -> Result<SubscriptionsSyncResponse, Error> {
        ...
        os_log(.debug, log: .hub, "Retrieving subscriptions")
        ...
        case .success(let data, _):
            os_log(.debug, log: .hub, "Successfully retrieved subscriptions")
            jsonData = data
        case .error(let error, _):
            os_log(
                .error,
                log: .hub,
                "Failed to fetch subscriptions from %@: %@",
                endpoint.absoluteString,
                error?.debugDescription ?? "unknown reason"
            )
        ...
        do {
            ...
            os_log(
                .error,
                log: .hub,
                "Failed to decode subscriptions response: %@, response body: %@",
    ...
    fileprivate func getPostsPage(cursor: String? = nil) async -> Result<PostsSyncResponse, Error> {
        ...
        os_log(
            .debug,
            log: .hub,
            "Retrieving a page of posts with cursor: \(cursor ?? "nil")"
        )
        ...
        case .success(let data, _):
            os_log(
                .debug,
                log: .hub,
                "Successfully retrieved a page of posts"
            )
            jsonData = data
        case .error(let error, _):
            os_log(
                .error, log: .hub,
                "Failed to fetch posts page from %@: %@",
                url.absoluteString,
                error?.debugDescription ?? "unknown reason"
            )
            return .failure(SyncError(message: error?.localizedDescription ?? "unknown reason"))
        ...
        do {
            ...
            os_log(
                .error, log: .hub,
                "Failed to decode posts response: %@, response body: %@",
                error.debugDescription,
                responseBodyString
            )
            return .failure(SyncError(message: error.localizedDescription))
