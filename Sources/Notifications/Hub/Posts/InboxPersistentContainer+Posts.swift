...
extension InboxPersistentContainer {
    ...
    static func fetchPosts(
        forSubscriptionID subscriptionID: String? = nil,
    ...
    func fetchPostByID(uuid: UUID) -> Post? {
        ...
        do {
            ...
            os_log(
                "Failed to fetch post by UUID: %{private}@",
                log: .hub,
                type: .error,
    ...
    // MARK: - SyncStatus Operations

    @MainActor
    func getPostsCursor() -> String? {

        let request = syncStatusRequest(for: .posts)
        ...
        do {
            ...
            os_log(
                "Failed to fetch posts cursor: %{private}@",
                log: .hub,
                type: .error,
                error.localizedDescription
            )
    ...
    /// Updates the posts sync cursor. Posts sync is forward-only and does not use
    /// backwards pagination or history-complete semantics.
    @MainActor
    func updatePostsSyncStatus(cursor: String?) throws {
        ...
        if let existingCursor = results.first {
            ...
            let newSyncStatus = SyncStatus(context: self.viewContext)
            newSyncStatus.roverEntity = SyncEntity.posts.rawValue
            newSyncStatus.cursor = cursor
        }
    }

    // MARK: - Drop (410 Reset)

    /// Surgically drops all posts data: Post entities and the posts sync cursor. Leaves
    /// `Subscription` entities untouched — they are upserted by ID, have no cursor, and
    /// self-heal on the next sync (parallel to how `dropAllConversations()` leaves posts
    /// untouched). Does not bump `conversationStoreGeneration` itself; the shared epoch is
    ...
    @MainActor
    func dropAllPosts() {
        let posts = (try? viewContext.fetch(Post.fetchRequest())) ?? []
        posts.forEach { viewContext.delete($0) }

        // Delete the posts forward cursor.
        if let postsStatus = try? viewContext.fetch(syncStatusRequest(for: .posts)).first {
            viewContext.delete(postsStatus)
        }
        ...
        do {
            ...
            os_log(
                "Failed to drop all posts: %{private}@",
                log: .hub,
                type: .error,
                error.localizedDescription
            )
    ...
    func markPostAsRead(_ post: Post) {
        ...
        do {
            ...
            os_log(
                "Failed to mark post as read: %{private}@",
                log: .hub,
                type: .error,
                error.localizedDescription
            )
    ...
    func createOrUpdatePost(from postItem: PostItem) -> Bool {
        ...
        do {
            ...
            if let subscriptionID = postItem.subscriptionID {
                if let existingSubscription = fetchSubscriptionByID(subscriptionID) {
            ...
            os_log(
                "Failed to create/update post: %{private}@",
                log: .hub,
                type: .error,
                error.localizedDescription
            )
    ...
    func loadSampleData() {
        ...
        scoreUpdates.status = "published"
        scoreUpdates.optIn = true
        ...
        gameDayGuide.status = "published"
        gameDayGuide.optIn = true
        ...
        do {
            ...
            os_log(
                "Failed to load sample data: %{private}@",
                log: .hub,
                type: .error,
                error.localizedDescription
            )
