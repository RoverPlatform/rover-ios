...
extension InboxPersistentContainer {
  ...
  func upsertSubscriptions(_ subscriptionItems: [SubscriptionItem]) {
    ...
    do {
      ...
      os_log(
        "Successfully upserted %d subscriptions", log: .hub, type: .debug,
        subscriptionItems.count)
    } catch {
      os_log(
        "Failed to upsert subscriptions: %@", log: .hub, type: .error,
  ...
  func fetchSubscriptionByID(_ id: String) -> Subscription? {
    ...
    do {
      ...
      os_log(
        "Failed to fetch subscription by ID: %@", log: .hub, type: .error,
        error.localizedDescription)
      return nil
    }
  }
