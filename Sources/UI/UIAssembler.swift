...
extension UIAssembler: Assembler {
    public func assemble(container: Container) {
        ...
        container.register(Action.self, name: "presentView", scope: .transient) {
            (_, viewControllerToPresent: UIViewController) in
            PresentViewAction(
        ...
        container.register(Router.self) { resolver in
            ...
            return RouterService(
                associatedDomains: self.associatedDomains,
    ...
    public func containerDidAssemble(resolver: Resolver) {
        ...
        if let authContext = resolver.resolve(AuthenticationContext.self) {
            for domain in associatedDomains {
                authContext.enableSDKAuthIDTokenRefreshForDomain(pattern: domain)
            }
        }
    }
}
