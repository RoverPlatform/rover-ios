...
/// Tests for InboxSync triggers and SDK integration
final class InboxSyncTriggerTests: InboxSyncTestBase {
  ...
  func testAppLaunchSync() async {
    ...
    // Simulate app launch by directly calling sync on InboxSync
    // In real app, this would be triggered by SyncCoordinator during app launch
    let syncResult = await inboxSync.sync()
    ...
    // This is handled internally by InboxSync.performActualSync()
  ...
  func testBackgroundPushSync() async {
    ...
    let partialSyncResult = await inboxSync.sync()
    XCTAssertFalse(
  ...
  func testManualRefreshSync() async {
    ...
    let refreshResult = await inboxSync.sync()
    ...
    let noDataRefreshResult = await inboxSync.sync()
    XCTAssertFalse(
  ...
  func testOfflineQueueing() async {
    ...
    let offlineSyncResult = await inboxSync.sync()
    ...
    let partialNetworkResult = await inboxSync.sync()
