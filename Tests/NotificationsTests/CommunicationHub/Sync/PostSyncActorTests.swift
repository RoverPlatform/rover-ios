...
/// Tests for PostSync actor concurrency and request coalescing behavior
final class PostSyncActorTests: PostSyncTestBase {
    ...
    func testConcurrentSyncRequestsCoalesce() async {
        ...
        let results = await withTaskGroup(of: Bool.self) { group in
            group.addTask { await self.postSync.sync() }
            group.addTask { await self.postSync.sync() }
            group.addTask { await self.postSync.sync() }
        ...
        XCTAssertEqual(
            finalPostsCallCount - initialPostsCallCount,
            1,
    ...
    func testSyncCompletionClearsActiveSyncTask() async {
        ...
        let startTime1 = Date()
        let secondResult = await postSync.sync()
        ...
        XCTAssertLessThan(
            elapsed1,
            1.0,
            "Second sync should complete quickly if activeSyncTask was cleared"
        )
        XCTAssertLessThan(
            elapsed2,
            1.0,
            "Recovery sync should complete quickly if activeSyncTask was cleared"
        )
    }
}
