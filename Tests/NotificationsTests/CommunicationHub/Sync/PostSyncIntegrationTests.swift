...
/// Tests for PostSync integration with SyncStandaloneParticipant
final class PostSyncIntegrationTests: PostSyncTestBase {
    ...
    func testSyncCoordinatorIntegration() async {
        ...
        // Test that PostSync can be called through the protocol interface
        let standaloneParticipant: SyncStandaloneParticipant = postSync
        ...
        XCTAssertEqual(
            results,
            [false, false, false],
            "All coalesced syncs should return the same result"
        )

        // Verify only one posts API call was made (coalescing worked)
    ...
    func testSyncParticipantReturnValues() async throws {
        ...
        let successResult = await postSync.sync()
        ...
        let noDataResult = await postSync.sync()
        ...
        let failureResult = await postSync.sync()
        XCTAssertFalse(failureResult, "Sync should return false when both APIs fail")

        // Test sync returns false when the posts request fails
        URLProtocolMock.reset()
        configureMockForFailure(postsError: URLError(.networkConnectionLost))

        let postsFailureResult = await postSync.sync()
        XCTAssertFalse(
            postsFailureResult,
            "Sync should return false when the posts request fails"
        )
        ...
        let firstCall = await postSync.sync()
        let secondCall = await postSync.sync()
        let thirdCall = await postSync.sync()
        ...
        try await clearAllData()  // Clear data to test final verification in isolation
        URLProtocolMock.reset()
        ...
        let verificationResult = await postSync.sync()
        ...
        XCTAssertEqual(
            actualSavedPosts.count,
            3,
            "Saved posts count should match expectation when sync returns true"
        )
    }

    /// Test isolation from main SDK sync operations
    func testIsolationFromMainSyncOperations() async {
        // Test that PostSync operates independently of other sync participants
        ...
        // Perform Hub sync
        let hubResult = await postSync.sync()
        ...
        async let isolatedSync1 = postSync.sync()
        async let isolatedSync2 = postSync.sync()
        async let isolatedSync3 = postSync.sync()
        ...
        XCTAssertEqual(
            persistenthubPosts.count,
            3,
            "Existing Hub posts should remain after sync failure"
        )
        XCTAssertEqual(
            persistenthubSubs.count,
            2,
            "Existing Hub subscriptions should remain after sync failure"
        )

        // Test that Hub operates on a separate posts endpoint
        // The Hub uses the REST /posts endpoint rather than the main SDK's GraphQL sync endpoint.
