...
/// Tests for PostSync cursor-based pagination logic
final class PostSyncPaginationTests: PostSyncTestBase {
    ...
    func testSinglePageSync() async {
        ...
        URLProtocolMock.stubPosts(
            testPosts,
            included: PostsSyncResponse.IncludedData(subscriptions: testSubscriptions),
            hasMore: false,
            nextCursor: nil
        )
    ...
    func testMultiPageRecursiveSync() async {
        ...
        URLProtocolMock.stubPaginationScenario { request in
            ...
            if cursor == nil {
                ...
                let response = PostsSyncResponse(
                    posts: paginatedResponses[0].posts,
                    included: nil,
                ...
                // Second page
                let response = PostsSyncResponse(
                    posts: paginatedResponses[1].posts,
                    included: nil,
                ...
                // Third page (final)
                let response = PostsSyncResponse(
                    posts: paginatedResponses[2].posts,
                    included: nil,
                    nextCursor: nil,
                    hasMore: false
                )
        ...
        XCTAssertEqual(
            savedPosts.count,
            12,
            "Should save all 12 posts from all 3 pages (4 posts × 3 pages)"
        )
        ...
        XCTAssertEqual(
            postsCalls[1].url?.queryParameters?["cursor"],
            "cursor-page-1",
            "Second call should have cursor from page 1"
        )
        XCTAssertEqual(
            postsCalls[2].url?.queryParameters?["cursor"],
            "cursor-page-2",
    ...
    func testPaginationStackOverflowPrevention() async {
        ...
        URLProtocolMock.stubPaginationScenario { request in
            ...
            if cursor == nil {
                ...
                if let pageNumber = Int(pageNumberString), pageNumber < paginatedResponses.count {
                    ...
                    let response = PostsSyncResponse(
                        posts: paginatedResponses[pageNumber].posts,
                        included: nil,
        ...
        XCTAssertEqual(
            savedPosts.count,
            1000,
            "Should save all 1000 posts from all 100 pages (10 posts × 100 pages)"
        )
        ...
        XCTAssertNil(postsCalls[0].url?.queryParameters?["cursor"], "First call should have no cursor")
        XCTAssertEqual(
            postsCalls[50].url?.queryParameters?["cursor"],
            "cursor-page-50",
            "Middle call should have correct cursor"
        )
        XCTAssertEqual(
            postsCalls[99].url?.queryParameters?["cursor"],
            "cursor-page-99",
    ...
    func testEmptyPageHandling() async throws {
        ...
        // When: Sync is performed
        let result2 = await postSync.sync()
    ...
    /// Verify every posts request includes `include=subscriptions` in its query parameters
    func testPostsRequestIncludesSubscriptionsQueryParam() async {
        URLProtocolMock.stubPosts([], hasMore: false)
        _ = await postSync.sync()
        assertPostsRequestsIncludeSubscriptions(URLProtocolMock.getCallLog())
    }
}
