...
/// Tests for RCHSync actor concurrency and request coalescing behavior
final class RCHSyncActorTests: RCHSyncTestBase {
  ...
  func testConcurrentSyncRequestsCoalesce() async {
    ...
    let results = await withTaskGroup(of: Bool.self) { group in
      group.addTask { await self.rchSync.sync() }
      group.addTask { await self.rchSync.sync() }
      group.addTask { await self.rchSync.sync() }
  ...
  func testSyncCompletionClearsActiveSyncTask() async {
    ...
    let startTime1 = Date()
    let secondResult = await rchSync.sync()
